...
export default function SellPage() {  
  ...
  const analyzeImage = async (imageData: string) => {  
    ...
    try {  
      ...
      // Call real API  
      const result = await api.ai.listingFromImage(base64);  
  ...
  const handlePublish = async () => {  
    if (!editedResult || !imagePreview) return;  
  
    try {  
      // In production, upload image to storage (S3, Azure Blob, etc.)  
      // For now, we'll use the base64 as imageUrl  
      ...
      await api.listings.create({  
        ...
        imageUrls: [imageUrl],  
        priceMin: editedResult.priceMin,  
        priceMax: editedResult.priceMax,  
        type: "Product",  
        latitude: undefined,  
        longitude: undefined,  
      });  
